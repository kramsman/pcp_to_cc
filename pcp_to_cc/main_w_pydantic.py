...
Usage (local dev):
    python pcp_to_cc/main_w_pydantic.py   # start Flask dev server
...
# Unknown included types are ignored via the fallback
...
class PcpPersonResponse(BaseModel):
    data: PcpPersonData
    included: list[Any] = []  # parsed manually below for flexibility

    def to_person_dict(self) -> dict:
        """
        Return the same flat dict that parse_person() returns:
            ...
            if item_type == "Email":
                ...
                if addr:
                    ...
                    if item_attrs.get("primary"):
                        continue  # found primary, keep scanning for field data

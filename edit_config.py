...
TABS = [
    ...
    },
    {
        ...
        "description":   "Lets a workflow require items that can be done at ANY time, which PCO cannot do on "
                         ...
                         "\n"
                         "A GATED STEP SHOULD ONLY WAIT. Because the card is released automatically, the step "
                         "an item names must do nothing but wait for that item — put the work that depends on "
                         "it in the NEXT step. So 'obtain bio' (gated by 'obtain bio!Raw Bio') is followed by "
                         "'Edit bio', which has no items and waits for a person. If you instead gate 'Edit bio' "
                         "directly, the card sails past it the moment the bio arrives and the editing never "
                         "gets done.\n"
                         "\n"
